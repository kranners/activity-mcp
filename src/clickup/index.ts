...
import { GetFilteredTeamTasksMetadataParam, GetFilteredTeamTasksResponse200 } from "@api/clickup/types";
...
export const ClickUpTasksAPIInputBody = {
  ...
  assignees: z.array(z.string()).describe("List of assignee IDs to filter by.").optional(),
  project_ids: z.array(z.string()).describe("List of project IDs to filter by.").optional(),
  space_ids: z.array(z.string()).describe("List of space IDs to filter by.").optional(),
  list_ids: z.array(z.string()).describe("List of list IDs to filter by.").optional(),
  date_updated_gt: z.number().int().describe("Filter by date updated greater than Unix time in milliseconds.").optional(),
  date_updated_lt: z.number().int().describe("Filter by date updated less than Unix time in milliseconds.").optional(),
};

const ClickUpTasksAPIInputBodySchema = z.object(ClickUpTasksAPIInputBody);

export type ClickUpTasksAPIInputBody = z.infer<typeof ClickUpTasksAPIInputBodySchema>;
...
const processParams = (params: ClickUpTasksAPIInputBody): ClickUpTasksAPIInputBody => {
